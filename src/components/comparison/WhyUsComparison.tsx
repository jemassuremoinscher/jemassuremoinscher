              ...
              <thead>
                <tr className="border-b border-border/50">
                  ...
                  <th className="p-4 text-center bg-primary/5">
                    <span className="text-lg font-bold text-primary">jemassuremoinscher</span>
                  </th>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="flex flex-col md:flex-row items-center justify-center gap-4 mb-12">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="relative bg-accent/10 border border-accent/30 rounded-xl px-5 py-3 max-w-sm">
            ...
            <p className="text-sm md:text-base font-medium text-foreground">
              ...
              {t('whyUs.save')}{" "}
              <span className="text-accent font-bold">250€ {t('whyUs.perYear')}</span> ?
            </p>
          </motion.div>
        </motion.div>
        ...
        <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
            ...
            <div className="flex flex-col items-center md:items-start gap-4">
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map(star => (
                    <Star key={star} className={`w-6 h-6 md:w-7 md:h-7 ${star <= 4 ? "fill-yellow-400 text-yellow-400" : "fill-yellow-400/80 text-yellow-400/80"}`} />
                  ))}
                </div>
              </div>
              <div className="text-center md:text-left">
                <p className="text-3xl md:text-4xl font-black text-foreground mb-1">4.8<span className="text-xl text-muted-foreground">/5</span></p>
                <p className="text-sm md:text-base text-muted-foreground">{t('whyUs.googleReviews')}</p>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="bg-gradient-to-br from-accent/10 to-accent/5 rounded-xl p-6 shadow-sm border border-accent/20 hover:shadow-md transition-shadow">
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <div>
                <p className="text-lg md:text-xl font-bold text-foreground">{t('whyUs.service100')}</p>
                <p className="text-2xl md:text-3xl font-black text-accent">{t('whyUs.freeService')}</p>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl p-6 shadow-sm border border-primary/20 hover:shadow-md transition-shadow">
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <div>
                <p className="text-lg md:text-xl font-bold text-foreground">{t('whyUs.data')}</p>
                <p className="text-2xl md:text-3xl font-black text-primary">{t('whyUs.secured')}</p>
              </div>
            </div>
          </motion.div>
