    ...
    <section className="py-16 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <Badge className="mb-4"><TrendingUp className="w-4 h-4 mr-2" />{t('blogHighlights.badge')}</Badge>
          <h2 className="text-3xl md:text-4xl font-bold mb-4">{t('blogHighlights.title')}</h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">{t('blogHighlights.subtitle')}</p>
        </div>
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          {featuredArticles.map((article) => (
            <Card key={article.id} className="hover:shadow-elegant transition-all hover-lift cursor-pointer group h-full flex flex-col" onClick={() => navigate(`/blog/${article.slug}`)}>
              <CardHeader className="flex-grow">
                ...
                <CardDescription className="text-muted-foreground line-clamp-3 mt-2">{article.description}</CardDescription>
              </CardHeader>
              <CardContent className="pt-0">
                ...
                <Button variant="ghost" className="w-full group-hover:bg-primary/10" onClick={(e) => { e.stopPropagation(); navigate(`/blog/${article.slug}`); }}>
                  {t('blogHighlights.readArticle')}<ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
                </Button>
              </CardContent>
            </Card>
          ))}
        ...
        <div className="text-center">
          <Button size="lg" variant="outline" onClick={() => navigate('/blog')} className="gap-2">{t('blogHighlights.viewAll')}<ArrowRight className="h-5 w-5" /></Button>
        </div>
        <div className="mt-12 p-6 bg-gradient-subtle rounded-xl">
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 text-sm">
            <div>
              <h3 className="font-semibold mb-3 text-primary">{t('blogPage.guidesTitle')}</h3>
              <ul className="space-y-2 text-muted-foreground">
                <li><button type="button" onClick={() => navigate('/blog/meilleure-assurance-auto-2026')} className="bg-transparent border-0 p-0 text-left cursor-pointer hover:underline">• Meilleure assurance auto 2026</button></li>
                <li><button type="button" onClick={() => navigate('/blog/top-mutuelles-sante-2026')} className="bg-transparent border-0 p-0 text-left cursor-pointer hover:underline">• Top mutuelles santé</button></li>
                <li><button type="button" onClick={() => navigate('/blog/assurance-auto-jeune-conducteur-astuces')} className="bg-transparent border-0 p-0 text-left cursor-pointer hover:underline">• Assurance jeune conducteur</button></li>
                <li><button type="button" onClick={() => navigate('/blog/comparatif-habitation-2026')} className="bg-transparent border-0 p-0 text-left cursor-pointer hover:underline">• Comparatif habitation</button></li>
              </ul>
            </div>
            <div>
              ...
              <ul className="space-y-2 text-muted-foreground">
                ...
                <li><button type="button" onClick={() => navigate('/blog/110-chevaux-pour-jeune-conducteur-en-2026-le-guide-complet')} className="bg-transparent border-0 p-0 text-left cursor-pointer hover:underline">• Jeune + voiture puissante</button></li>
