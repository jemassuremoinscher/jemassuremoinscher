...
export const SimplifiedLeadForm = ({ insuranceType, insuranceLabel }: SimplifiedLeadFormProps) => {
  ...
  const onSubmit = async (data: SimplifiedLeadData) => {
    ...
    try {
      ...
      const { error } = await supabase.from('insurance_quotes').insert({ insurance_type: insuranceType, full_name: data.fullName, email: data.email, phone: data.phone, quote_data: { source: `landing_${insuranceType}`, utm_data: utmData }, status: 'pending' });
