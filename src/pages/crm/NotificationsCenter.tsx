...
export default function NotificationsCenter() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-slate-900 dark:text-slate-50">
            <Bell className="h-6 w-6 text-[#7C3AED] dark:text-[#C4B5FD]" /> Centre de notifications
          </h1>
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full border border-[#E9D5FF] dark:border-[#362B54] px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-[#FAF5FF] dark:hover:bg-[#262140] disabled:opacity-50"
          >
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-3xl border border-[#E9D5FF] dark:border-[#362B54] bg-white dark:bg-[#1E1B2E] p-3">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            ...
            className="w-full rounded-full border border-[#E9D5FF] dark:border-[#362B54] bg-[#FAF5FF]/40 dark:bg-[#13111C]/40 py-2 pl-9 pr-3 text-sm"
          />
        </div>
        <select
          value={typeFilter}
          onChange={(e) => setTypeFilter(e.target.value)}
          className="rounded-full border border-[#E9D5FF] dark:border-[#362B54] bg-white dark:bg-[#1E1B2E] px-3 py-2 text-sm"
        >
      ...
      <div className="rounded-3xl border border-[#E9D5FF] dark:border-[#362B54] bg-white dark:bg-[#1E1B2E]">
        {loading ? (
          <div className="p-8 text-center text-sm text-slate-500 dark:text-slate-400">Chargement…</div>
        ) : filtered.length === 0 ? (
          <div className="p-8 text-center text-sm text-slate-500 dark:text-slate-400">Aucune notification.</div>
        ) : (
          <ul className="divide-y divide-slate-100 dark:divide-[#362B54]">
            {filtered.map((r) => (
              <li
                key={r.id}
                className={`flex items-start gap-3 p-4 transition ${
                  r.read_at ? "bg-white dark:bg-[#1E1B2E]" : "bg-[#FAF5FF]/60 dark:bg-[#13111C]/60"
                }`}
              >
                <div className={`mt-1 h-2 w-2 shrink-0 rounded-full ${r.read_at ? "bg-slate-300 dark:bg-slate-600" : "bg-[#7C3AED]"}`} />
                <button
                  type="button"
                ...
                >
                  ...
                  {r.body && <div className="text-xs text-slate-600 dark:text-slate-300">{r.body}</div>}
                ...
                <button
                  type="button"
                  onClick={() => remove(r.id)}
                  className="rounded-full p-2 text-slate-400 dark:text-slate-500 hover:bg-red-50 dark:hover:bg-red-950/40 hover:text-red-600 dark:hover:text-red-400"
