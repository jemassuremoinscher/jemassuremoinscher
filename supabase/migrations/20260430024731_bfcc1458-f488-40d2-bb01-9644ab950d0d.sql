...
create index if not exists idx_linkedin_auto_posts_pending_schedule
on public.linkedin_auto_posts (status, scheduled_at, created_at);
...
begin
  ...
  if v_linkedin_enabled then v_channels := v_channels || jsonb_build_array('linkedin'); end if;
  if v_facebook_enabled then v_channels := v_channels || jsonb_build_array('facebook'); end if;
  ...
  select id into v_existing_id from public.linkedin_auto_posts where article_slug = new.slug limit 1;
  ...
  if v_existing_id is null then
    insert into public.linkedin_auto_posts (
      ...
      provider, status, linkedin_status, facebook_status, posted_at, scheduled_at, error_message, response_payload
    ) values (
...
exception when others then
  insert into public.linkedin_auto_posts (
    article_slug, article_title, article_url, image_url, short_description, post_content,
    provider, status, linkedin_status, facebook_status, scheduled_at, error_message, response_payload
  ) values (
    ...
    new.suggested_meta_description, 'make', 'failed', 'failed', 'failed', coalesce(v_published_at, new.published_at), sqlerrm,
